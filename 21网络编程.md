...
=======
	EPOLL_CTL_MOD，再次设置事件
>>>>>>> 9d03336998ea89a23247759bb4e82c7b58be918b
#### code
```c
/* 基于epoll的回声服务器 */
#include<stdio.h>
...
reactor就是对epoll进行封装，进行网络IO与业务的解耦，将epoll管理IO变成管理事件，整个程序由事件进行驱动执行。reactor：由事件驱动执行对应的回调函数；epoll：需要自己判断。
...
# 2.1.3 http/https 服务器的实现
## reactor sendbuffer recvbuffer 封装http 协议
...
#define HTTP_WEBSERVER_HTML_ROOT	"html"  //html文件的存储文件夹
 
 
#define HTTP_METHOD_GET		0 
#define HTTP_METHOD_POST	1//定义宏控制对不同http请求进行处理
...
struct ntyevent {
	...
	// http param
	int method; //
	char resource[BUFFER_LENGTH];//这里的长度是根据请求的最大长度定义的，这里假定的是4K
	int ret_code;
...
int recv_cb(int fd, int events, void *arg) {
	...
	if (len > 0) {
		...
		http_request(ev);//用来对HTTP请求进行判断，如果是HTTP请求，根据不同的状态，分别配不同的状态机（这里状况很多，先不会用考虑，之后结合nginx学习）
...
int http_response(struct ntyevent *ev) {
	...
	if (filefd == -1) { // return 404
		...
		struct stat stat_buf;//linux系统中获取文件属性的API
		fstat(filefd, &stat_buf);
		close(filefd);
 
		if (S_ISDIR(stat_buf.st_mode)) {//判断请求的是路径？
		...
		} else if (S_ISREG(stat_buf.st_mode)) {//判断请求的是文件？
 
			ev->ret_code = 200;//状态码：正常收发
...
int send_cb(int fd, int events, void *arg) {
	...
	if (len > 0) {
		...
		if (ev->ret_code == 200) {
			...
			sendfile(fd, filefd, NULL, stat_buf.st_size);//零拷贝，相当于磁盘里文件直接通过socketfd发送
...
- **http 协议和 reactor 的关系：**
  - reactor 的结构可以拆分成如下的主要结构
    - ntyreactor_run 提供长时间运行
    - accept_cb
    - recv_cb 接收http 请求
    - send_cb 返回http 响应
- **http 的tcp 连接的生命周期：**
  - accept_cb --->recv_cb --->send_cb
- **结合http的tcp生命周期来分析我们需要对accept_cb做的处理：**
  - 我们如何保证http 请求报文读取完？
    - 分隔符accpet_cb \r\n\r\n（基于应用层数据先发的先到，后发的后到。ps：协议栈内不遵守这个规则）
    - 定义包的长度
- **结合我们制作的reactor，我们赋予它处理HTTP请求的能力(新增用来处理http的函数)：**
  - 结构体ntyevent 加入对 HTTP 的状态存储、以及存储buffer
  - accept_cb() 与对所有fd的处理相同
  - recv_cb() ——新增http_request() 处理
  - send_cb() ——新增http_response() 处理
  - ps: 此代码主要为了展现以reactor 如何对http 处理进行修改，为之后看其他框架做基础
...
**缺点：**
   ...
   客户端必须为每一个待请求的对象建立并维护一个新的连接，也就是说，当页面存在多个对象，http1.0建立非持久连接，使得一个页面下载十分缓慢，增加了网络传输负担
...
**新增：**
1.在http1.0的基础上引用了更多缓存控制策略
2.在引入范围内设置，优化了带宽
3.在错误的通知中新增了错误状态响应码
4.增加了HOST头处理，可传递主机名(hostname)

**缺点：**
传输内容时明文，不安全

> HTTPS
> 指HTTP运行在安全套接字协议(SSL)或传输层安全协议(TLS)之上，在TCP中传输的内容都要加密
> https和http的区别：
> 1.https传输的是经过SSL层加密的数据，而http传输的明文，安全性有差异
> 2.http因为传输的是明文，所以浏览器接收速度快，而https传输加密数据，浏览器需要解密传输速率慢，而且耗电量会增加10%~20%
> 3.https因为需要加密，服务器需要给浏览器证书，而安全性越高的证书需要的费用越昂贵
> 有限状态机 fsm 解析 http
...
> 1.在Http1.x中浏览器限制了同一个域名下的请求数量，当在请求很多资源时，剩余的资源需要等待当前的六个请求完成后才能发起请求。
>
> 2.Http2中引用了多路复用的技术，这个技术可以只通过一个TCP连接就可以传输所有的请求数据。多路复用可以绕过浏览器限制同一个域名下的请求数量问题，进而提高了网页的性能
...
> 关于TCP协议与UDP协议的区别
>
> 1.TCP是面向连接的，UDP是无连接的，也就是说是不需要提前建立连接的
>
> 2.TCP提供可靠的服务，通过TCP连接的服务，无差错，不丢失，不重复，并且 按照一定的顺序到达，UDP是尽自己最大的努力交付，不保证可靠性交付。所以TCP适合大数据量交换
>
> 3.TCP是面向字节流，UDP是面向报文，并且，网络出现拥塞，不会使得发送效率降低。
>
> 4.TCP只能是1对1的，而UDP可以1对1，也可以1对多
>
